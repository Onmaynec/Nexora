...
function mountV3Features(options) {
  ...
  app.put("/api/v3/drafts/:conversationId", authRequired, async (request, response) => {
    ...
    if (conversationUsesMls(conversation.id)) return fail(response, 409, "Черновик MLS хранится только в зашифрованном локальном хранилище.", "LEGACY_READ_ONLY");
  ...
  app.post("/api/messages/scheduled", authRequired, async (request, response) => {
    ...
    if (conversationUsesMls(conversation.id)) return fail(response, 409, "Отложенная plaintext-отправка недоступна в MLS-диалоге.", "LEGACY_READ_ONLY");
  ...
  app.post("/api/conversations/:id/polls", authRequired, async (request, response) => {
    ...
    if (!canAccessConversation(state, conversation, request.nexora.user.id)) return fail(response, 403, "Чат недоступен.", "FORBIDDEN");
    if (conversationUsesMls(conversation.id)) return fail(response, 409, "Опросы требуют отдельного E2EE-формата и пока недоступны в MLS-диалоге.", "LEGACY_READ_ONLY");
  ...
  app.post("/api/v3/bot/messages", botAuthentication, async (request, response) => {
    ...
    if (conversationUsesMls(conversation.id)) return fail(response, 409, "Бот не имеет доверенного MLS-устройства и не может отправлять plaintext.", "LEGACY_READ_ONLY");
    try {
  ...
  app.post("/api/conversations/:id/uploads", authRequired, async (request, response) => {
    ...
    if (!canAccessConversation(state, conversation, request.nexora.user.id)) return fail(response, 403, "Чат недоступен.", "FORBIDDEN");
    if (conversationUsesMls(conversation.id)) return fail(response, 409, "Вложения в MLS-диалоге должны быть зашифрованы на клиенте.", "LEGACY_READ_ONLY");
  ...
  app.put("/api/uploads/:id/chunks/:index", authRequired, express.raw({ type: "application/octet-stream", limit: CHUNK_BYTES + 1024 }), async (request, response) => {
    ...
    if (upload && conversationUsesMls(upload.conversationId)) return fail(response, 409, "Загрузка отменена: диалог переведён на MLS E2EE.", "LEGACY_READ_ONLY");
  ...
  app.post("/api/uploads/:id/complete", authRequired, async (request, response) => maintenance.withFileLock(async () => {
    ...
    if (upload && conversationUsesMls(upload.conversationId)) {
      ...
      return fail(response, 409, "Загрузка отменена: диалог переведён на MLS E2EE.", "LEGACY_READ_ONLY");
    }
