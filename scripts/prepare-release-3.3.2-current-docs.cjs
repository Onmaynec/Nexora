...
replaceRequired("SECURITY.md", [
  ["| `3.3.1` | Published `UNSIGNED-TEST` prerelease | Текущая поддерживаемая prerelease-линия; security fixes принимаются |", "| `3.3.2` | Published `UNSIGNED-TEST` prerelease | Текущая поддерживаемая prerelease-линия; security fixes принимаются |"],
  ["| `3.2.0–3.3.0` | Superseded prereleases | Обновитесь до `3.3.1`; отчёты принимаются для regression/impact analysis |", "| `3.2.0–3.3.1` | Superseded prereleases | Обновитесь до `3.3.2`; отчёты принимаются для regression/impact analysis |"],
  ["## 5. Текущая security boundary — 3.2.4", "## 5. Текущая security boundary — 3.3.0+"],
  ["[Security Review 3.2.4](SECURITY_REVIEW_3.2.4.md), [Release Verification 3.2.4](RELEASE_VERIFICATION_3.2.4.md)", "[Security Review 3.3.0](SECURITY_REVIEW_3.3.0.md), [Release Verification 3.3.2](RELEASE_VERIFICATION_3.3.2.md)"],
  ["signed stable Windows status для 3.2.4", "signed stable Windows status для 3.3.2"],
]);

replaceRequired("SUPPORT.md", [
  ["| `3.3.1` published `UNSIGNED-TEST` prerelease | Текущая prerelease-линия; defect и security reports принимаются |", "| `3.3.2` published `UNSIGNED-TEST` prerelease | Текущая prerelease-линия; defect и security reports принимаются |"],
  ["| `3.2.0–3.3.0` | Superseded prereleases; обновитесь до `3.3.1` перед обычной диагностикой |", "| `3.2.0–3.3.1` | Superseded prereleases; обновитесь до `3.3.2` перед обычной диагностикой |"],
...
replaceRequired("docs/DEPLOYMENT.md", [
...
replaceRequired("TESTER_GUIDE.md", [
...
replaceRequired("docs/PRODUCT_OVERVIEW.md", [
  ...
  ["- `3.3.1` — packaged Windows Server startup correction: `shared/**/*` включён в `app.asar` и защищён release gate.", "- `3.3.1` — packaged Windows Server startup correction: `shared/**/*` включён в `app.asar` и защищён release gate;\n- `3.3.2` — release metadata, current documentation, release history и published-asset smoke приведены к одному проверяемому состоянию."],
...
replaceRequired("docs/RELEASE_POLICY.md", [
...
replaceRequired("docs/GITHUB_RELEASE.md", [
...
replaceRequired("docs/RELEASE_CHECKLIST.md", [
  ["# Nexora 3.2.4 Release Checklist", "# Nexora 3.3.2 Release Checklist"],
...
replaceRequired(".github/ISSUE_TEMPLATE/bug_report.yml", [
  ["Current version: 3.3.1 published UNSIGNED-TEST prerelease. Signed production baseline: 3.1.2. Versions 3.2.0–3.3.0 are superseded; reproduce on 3.3.1 when possible.", "Current version: 3.3.2 published UNSIGNED-TEST prerelease. Signed production baseline: 3.1.2. Versions 3.2.0–3.3.1 are superseded; reproduce on 3.3.2 when possible."],
...
replaceRequired("website/index.html", [
  [">3.2.4<", ">3.3.2<"],
]);

replaceRequired("website/app.js", [
  ["const FALLBACK_VERSION = \"3.2.4\";", "const FALLBACK_VERSION = \"3.3.2\";"],
...
replaceRequired("website/site-fixes.js", [
  ["const FALLBACK_VERSION = \"3.3.0\";", "const FALLBACK_VERSION = \"3.3.2\";"],
]);

console.log("Synchronized all current Nexora 3.3.2 documentation surfaces.");
