...
async function main() {
  ...
  try {
    await instance.store.mutate((state) => {
      ...
      const room = { id: "soak-room", name: "Soak", slug: "soak", privacy: "private", ownerId: "soak-user", inviteCode: null, joinPolicy: "invite", readOnly: false, slowModeSeconds: 0, allowFiles: true, allowVoice: true, createdAt };
    ...
    console.log(`SOAK OK: ${minutes} min, ${cycles} cycles, integrity=${instance.store.integrityCheck().details}`);
  } finally {
